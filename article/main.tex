...
We reasserted the targeted attack scenario, but for the second time we only removed one of the $30$ largest hubs and recorded the number of connected components. As it can be seen in Figure~\ref{fig:vertecconnectivity1removed} most of the hubs (25!) are so critical that their removal would be catastrophic for LN, namely they would leave behind several disconnected components.
...
\subsection{Fixing LN's resilience against random failures and attacks}
